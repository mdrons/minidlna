...
parse_nfo(const char *path, metadata_t *m)
{
	...
	if (!nfo)
	{
		DPRINTF(E_DEBUG, L_METADATA, "ERROR opening .nfo file: %s\n", path);
		free(buf);
		return;
	}
	...
	DPRINTF(E_DEBUG, L_METADATA, "TITLE: val [GetValueFromNameValueList]: %s\n",val );
	if (val)
	{
		char *esc_tag, *title;
	...
	DPRINTF(E_DEBUG, L_METADATA, "PLOT:val [GetValueFromNameValueList]: %s\n",val );
	if (val)
	{
	...
	DPRINTF(E_DEBUG, L_METADATA, "NFO Data: %s %s %s\n",m->title,m->comment,m->genre);
